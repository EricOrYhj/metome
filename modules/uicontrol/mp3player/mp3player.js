...
define(function () {
    function MP3Player(_options) {
        ...
        if ($("#" + this.options.id).length == 0) {
            ...
            if ($.browser.msie) {
                if (typeof (Worker) == 'undefined') {
                    this.video.attachEvent("PlayStateChange", function (state) {
                        switch (state) {
                            case 1:
                    ...
                    this.video.attachEvent("ended", function () {
                        _this.options.onStop();
                    });
                    this.video.attachEvent("pause", function () {
                        _this.options.onPause();
                    });
                }
                ...
                if (typeof (Worker) == 'undefined') {
                    this.video.addEventListener("PlayStatusChange", function (state) {
                        switch (state) {
                            case 1:
                                _this.options.onStop();
                                break;
        ...
        this.getAllTime = function () {
            if (typeof (Worker) == 'undefined') {
                var media = this.video.currentMedia;
                if (media) {
